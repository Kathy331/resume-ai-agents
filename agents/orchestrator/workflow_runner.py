...
'''
    ...
    def run_enhanced_prep_guide(self, max_interviews: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            # Import the production prep guide generator
            try:
                from workflows.prep_guide_generator import generate_comprehensive_prep_guide
            except ImportError as e:
                print(f"❌ Could not import prep_guide_generator: {str(e)}")
                print("ℹ️  Prep guide generator module not available in current environment")
                return {
                    'success': False,
                    'error': f'Prep guide generator module not found: {str(e)}',
                    'guides_generated': 0,
                    'processing_time': (datetime.now() - start_time).total_seconds()
                }
    ...
    def run_individual_email_workflow(self, max_emails: int = 10) -> Dict[str, Any]:
        """
        Run the complete individual email processing workflow
        
        This method processes emails one by one through:
        1. Email Classification → 2. Entity Extraction → 3. Deep Research → 4. Prep Guide Generation
        
        Each email gets its own output file named by company keyword.
        
        Args:
            max_emails: Maximum number of emails to process
            
        Returns:
            Results including individual processing details
        """
        print(f"🚀 Individual Email Processing Workflow")
        print("=" * 60)
        start_time = datetime.now()
        
        try:
            # Import the individual email workflow
            from workflows.individual_email_workflow import IndividualEmailWorkflow
            
            # Create and run the workflow
            individual_workflow = IndividualEmailWorkflow()
            result = individual_workflow.run_complete_individual_workflow(max_emails)
            ...
            # Add to execution history
            self.execution_history.append({
                'type': 'individual_email_workflow',
                'timestamp': datetime.now().isoformat(),
                'success': result.get('success', False),
                'emails_processed': result.get('emails_processed', 0),
                'interviews_found': result.get('interviews_found', 0),
                'prep_guides_generated': result.get('prep_guides_generated', 0),
                'processing_time': processing_time
            })
            ...
            print(f"💥 Individual Email Workflow failed: {str(e)}")
            return error_result
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Run the new individual email workflow
        result = runner.run_individual_email_workflow(max_emails=10)
        
        if result.get('success'):
            print(f"\n🎉 Individual Email Workflow completed successfully!")
            print(f"📧 Emails Processed: {result.get('emails_processed', 0)}")
            print(f"🎯 Interview Emails: {result.get('interviews_found', 0)}")
            print(f"� Prep Guides Generated: {result.get('prep_guides_generated', 0)}")
            print(f"⏱️  Total Time: {result.get('total_processing_time', 0):.2f}s")
        else:
            print(f"❌ Individual Email Workflow failed: {result.get('error', 'Unknown error')}")
        ...
        for run in runner.get_execution_history():
            ...
            # Show additional details for individual email workflow
            if run_type == 'individual_email_workflow':
                emails = run.get('emails_processed', 0)
                interviews = run.get('interviews_found', 0)
                guides = run.get('prep_guides_generated', 0)
                print(f"  - {run_type}: {success_icon} ({processing_time:.1f}s) - {emails} emails, {interviews} interviews, {guides} guides")
            else:
                print(f"  - {run_type}: {success_icon} ({processing_time:.1f}s)")
        
        print(f"\n💾 Individual output files saved in: outputs/fullworkflow/")
